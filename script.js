...
function exportPDF() {
    ...
    doc.setFontSize(16);
    doc.text("ECG BIOMEDICAL PATIENT REPORT", 20, 20);

    doc.setFontSize(11);

    doc.text("PATIENT INFORMATION:", 20, 40);
    doc.text("Name: xxxxxxxxxxxxxxxxx", 20, 50);
    doc.text("Hospital: xxxxxxxxxxxxxxxxx", 20, 60);
    doc.text("Department: ICU", 20, 70);
    doc.text("Bed Number: xxxx", 20, 80);
    doc.text("Medical Doctor: xxxxxxxxxxxxxx", 20, 90);
    doc.text("Biomedical Technician: xxxxxxxxxx", 20, 100);

    doc.text("CLINICAL DATA:", 20, 120);
    doc.text(`BPM: ${bpm || "N/A"}`, 20, 130);
    doc.text(`Risk Index: ${risk}`, 20, 140);
    doc.text(`Condition: ${alarm}`, 20, 150);

    doc.text("IMMEDIATE ACTION:", 20, 170);

    let action =
        alarm === "NORMAL" ? "No immediate action required." :
        alarm === "WARNING" ? "Monitor patient closely and reassess." :
        "URGENT: Immediate medical intervention required.";

    doc.text(action, 20, 180);

    doc.text(`Date: ${now.toLocaleDateString()}`, 20, 200);
    doc.text(`Time: ${now.toLocaleTimeString()}`, 20, 210);
